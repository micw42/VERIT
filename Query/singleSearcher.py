...
def name_query(name, nodes, full_df, string_type):
    ...
    in_net = pd.merge(nodes, name_query, left_on="Only_Id", right_on="id", how="inner").iloc[:, 3:6].drop_duplicates().reset_index(drop=True)
    print("COLS:", in_net.columns)
    u_ids = list(in_net["id"].drop_duplicates())
    in_net = full_df.query("id == @u_ids").iloc[::-1].reset_index(drop=True)
    in_net = pd.merge(nodes, in_net, left_on="Only_Id", right_on="id", how="inner").iloc[:, 3:6].drop_duplicates().reset_index(drop=True)
    in_net = in_net.drop_duplicates(subset = ["name", "id"])
    in_net = in_net.sort_values(by = "PRValue", ascending = False)
